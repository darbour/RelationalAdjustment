...
for(trial in 1:50) {
  ...
  for(hyp.friends in hyp.friends.values) {
    ...
    estimates[, paste0("mf_", hyp.friends)] <- c(act.outcome, gp.est.outcome, lmI.est.outcome, lmII.est.outcome, NA)
  }
  estimates$mt_0 <- c(gendata$outcome.function(myt=0, sdnoise=0), gp.model(myt=0), lm.model(myt=0), lm.modelII(myt=0), NA)
  estimates$mt_1 <- c(gendata$outcome.function(myt=1, sdnoise=0), gp.model(myt=1), lm.model(myt=1), lm.modelII(myt=1), NA)
  estimates$global_treatment <- c(gendata$outcome.function(myt=1, friendt=1, sdnoise=0), gp.model(myt=1, friendt=1), lm.model(myt=1, friendt=1), lm.modelII(myt=1, friendt=1), htmeans$tmean)
  estimates$global_control <- c(gendata$outcome.function(myt=0, friendt=0, sdnoise=0), gp.model(myt=0, friendt=0), lm.model(myt=0, friendt=0), lm.modelII(myt=0, friendt=0), htmeans$cmean)
